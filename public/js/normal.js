...
async function init() {
  ...
  for (c of cuestionarios) {
    ...
    for (p of preguntas) {
      var nodoP = creaNodoPregunta(p.preguntaId, p.pregunta, p
        .respuesta);
    ...
    fetch(`${base}/createma`, {
      ...
      .then(res => {
        if (res.result) {
          ...
          var cuestionario = creaNodoCuestinario(temaId, tema
            .value);
          ...
          console.log(
            `Error al crear tema ${tema.value}: ${res.error}`
          );
...
function addPregunta(evento) {
  ...
  var r = document.querySelector(
    `input[name="${selector.id}_respuesta"]:checked`);
  ...
  if (p.value != '') {
    ...
    fetch(`${base}/tema/${temaId}/creapregunta`, {
      ...
      .then(res => {
        if (res.result) {
          selector.appendChild(creaNodoPregunta(res.result, p
            .value, r.value));
          p.value = "";
          document.querySelector(
              `input[name="${selector.id}_respuesta"]`)
            .checked = "true";
        } else {
          console.log(
            `Error creando pregunta en tema ${temaId}: ${res.error}`
          );
          window.alert(`Error creando pregunta en tema ${temaId}: ${res.error}`);
        }
      })
      .catch(error => {
        console.log(error);
  ...
  return fetch(`${base}/temas`, {
    ...
    .then(res => {
      if (res.result) {
        ...
        window.alert(`Error al cargar cuestionarios: ${res.error}`);
        return [];
      }
    })
    .catch(error => {
  ...
  return fetch(`${base}/tema/${temaId}/preguntas`, {
    ...
    .then(res => {
      if (res.result) {
        ...
        window.alert(`Error al cargar preguntas: ${res.error}`);
        return [];
      }
    })
    .catch(error => {
  ...
  fetch(`${base}/pregunta/${preguntaId}`, {
    ...
    .then(res => {
      if (res.result) {
          ...
          fetch(`${base}/tema/${temaId}`, {
            ...
            .then(res => {
              if (res.result) {
                ...
                console.log(
                  `Error al borrar el tema: ${res.error}`
                );
                window.alert(`Error al borrar el tema: ${res.error}`);
              }
            })
            .catch(error => {
              console.log(error);
        ...
        window.alert(`Error al borrar la pregunta: ${res.error}`);
      }
    })
    .catch(error => {
      console.log(error);
